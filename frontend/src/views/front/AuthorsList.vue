<template>
  <div class="max-w-4xl mx-auto px-4 py-12">
    <!-- Page Header -->
    <div class="mb-10">
      <h1 class="text-3xl font-bold mb-2" :style="{ color: 'var(--color-text)' }">✍️ 专栏作者</h1>
      <p class="text-sm" :style="{ color: 'var(--color-text-muted)' }">浏览所有作者的文章</p>
    </div>
    ...
    <h2 class="text-lg font-bold mb-4" :style="{ color: 'var(--color-text)' }">
      {{ search ? `搜索结果（${filteredAuthors.length}）` : '热门作者' }}
    </h2>
    ...
    <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
      ...
      >
        <span class="font-medium text-sm" :style="{ color: 'var(--color-text)' }">{{ author.display_name || author.username }}</span>
        <span class="text-xs px-2 py-0.5 rounded-full" :style="{ backgroundColor: 'var(--color-bg-hover)', color: 'var(--color-text-muted)' }">
          {{ author.post_count || 0 }}
        </span>
      </router-link>
    </div>
    <div v-if="!loading && displayAuthors.length === 0" class="py-12 text-center" :style="{ color: 'var(--color-text-muted)' }">
      {{ search ? '未找到匹配的作者' : '暂无作者' }}
    </div>
  </div>
</template>
...
const filteredAuthors = computed(() => {
  if (!search.value.trim()) return authors.value
...
const displayAuthors = computed(() => {
  ...
  // Top 10 authors with post_count > 0, sorted descending
  return authors.value
    .filter(a => (a.post_count || 0) > 0)
...
onMounted(async () => {
  loading.value = true
  try {
    const { data: resp } = await api.get<{ data: AuthorItem[] }>('/authors')
